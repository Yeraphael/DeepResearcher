  ...
  <main class="app-shell" :class="{ expanded: isExpanded }">
    ...
    <div v-else class="layout layout-fullscreen">
      ...
      >
        ...
        <p v-if="error" class="error-chip result-error">
          <svg viewBox="0 0 20 20" aria-hidden="true">
            <path
              d="M10 3.2c-.3 0-.6.2-.8.5L3.4 15c-.4.7.1 1.6.8 1.6h11.6c.7 0 1.2-.9.8-1.6L10.8 3.7c-.2-.3-.5-.5-.8-.5Zm0 4.3c.4 0 .7.3.7.7v4c0 .4-.3.7-.7.7s-.7-.3-.7-.7V8.2c0-.4.3-.7.7-.7Zm0 6.6a1 1 0 1 1 0 2 1 1 0 0 1 0-2Z"
            />
          ...
          <aside class="tasks-list">
            ...
            <ul>
              <li
                ...
                :key="task.id"
                :class="['task-item', { active: task.id === activeTaskId, completed: task.status === 'completed', failed: task.status === 'failed' }]"
              >
                <button
                  type="button"
                  class="task-button"
...
const error = ref("");
const runState = ref<"idle" | "running" | "success" | "failed" | "cancelled">(
  "idle"
);
...
const statusChipClass = computed(() => ({
  active: runState.value === "running",
  success: runState.value === "success",
  failed: runState.value === "failed",
  cancelled: runState.value === "cancelled"
}));
...
const handleSubmit = async () => {
  ...
  try {
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "done") {
          if (runState.value === "running") {
            runState.value = error.value ? "failed" : "success";
          }
          return;
        }
    ...
    if (!reportMarkdown.value && runState.value === "running") {
